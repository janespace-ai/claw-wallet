...
description: Start Claw Wallet Electron app - npm install in desktop/, dev script, config, troubleshooting (permissions, better-sqlite3, ELECTRON_RUN_AS_NODE). Use when user asks how to run desktop wallet or fix install/launch.
---
...
- Node.js：建议 20.x / 22.x LTS；若使用 24.x，本地编译原生依赖时可能需要 `CXXFLAGS=-std=c++20`（见下文）。
...
请勿在项目目录对 npm 使用 sudo，否则 node_modules 可能出现属主为 root 的文件，导致 EACCES、rename 失败。若已误操作：
...
- 本地链 RPC 需与 config.json 一致；Agent 端链与 RPC 需与桌面可连上的节点一致。

## 故障速查

- EACCES / rename better-sqlite3：修复 node_modules 属主或删除后不用 sudo 重装
- Electron 异常：使用 npm run dev；不要带着 ELECTRON_RUN_AS_NODE 手动启动
- prebuild 超时：重试或 CXXFLAGS 走本地编译
- npm warn Unknown env config devdir：检查 ~/.npmrc 并移除无效 devdir（可选）

## Agent 提示

用户说启动桌面钱包时：先 cd desktop，再 npm install / npm run dev。拉代码后先确认 desktop/node_modules 安装成功。
