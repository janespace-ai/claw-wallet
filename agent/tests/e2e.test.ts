...
describe("claw-wallet E2E (Phase 2 — no local signer)", () => {
  ...
  it("1. all 19 tools are registered (Phase 2: includes wallet_pair)", () => {
    const tools = wallet.getTools();
    const toolNames = tools.map((t) => t.name).sort();
    expect(toolNames).toEqual([
      "wallet_address", "wallet_approval_approve", "wallet_approval_list",
      "wallet_approval_reject", "wallet_balance", "wallet_call_contract",
      "wallet_contacts_add", "wallet_contacts_list", "wallet_contacts_remove",
      "wallet_contacts_resolve", "wallet_create", "wallet_estimate_gas",
      "wallet_history", "wallet_import", "wallet_pair", "wallet_policy_get",
      "wallet_policy_set", "wallet_send", "wallet_sign_typed_data",
    ]);
