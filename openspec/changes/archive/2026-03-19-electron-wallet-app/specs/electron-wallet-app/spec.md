...
The Electron App SHALL include an embedded signing engine that automatically signs transactions within the configured allowance budget and prompts user confirmation for transactions exceeding the budget.

#### Scenario: Auto-sign within budget
- **WHEN** a signing request arrives and the transaction amount is within the per-transaction limit AND the daily cumulative total is within the daily limit
...
- **THEN** the App SHALL allow configuring daily total limit (default 500 USDT), per-transaction limit (default 100 USDT), allowed tokens list, and allowed recipient addresses
