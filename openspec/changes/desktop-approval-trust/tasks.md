...
- [ ] **Desktop**：签名请求入口用 `PriceService` + 链上字段计算 `estimatedUsdForPolicy`；无价 → 禁止 `autoApprove`。
- [ ] **Desktop**：调整 `SigningEngine.checkBudget`（或等价）仅消费上述估值；移除或降级对 Agent `amountUsd`/误导字段的依赖。
