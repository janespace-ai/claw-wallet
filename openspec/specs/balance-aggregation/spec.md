...
The system SHALL cache balance data per account for 10 seconds to avoid redundant RPC calls.

#### Scenario: Recent balance cache hit
- **WHEN** user switches from Account 0 to Account 1 and back to Account 0 within 10 seconds
- **THEN** system displays cached balances without new RPC queries

#### Scenario: Balance cache expiration
- **WHEN** cached balance data is older than 10 seconds
- **THEN** system fetches fresh balances from RPC providers

#### Scenario: Manual balance refresh
- **WHEN** user clicks "Refresh" button
- **THEN** system invalidates cache and fetches fresh balances immediately
